	...
	apply(compiler) {
		...
		compiler.hooks.compilation.tap(
			...
			(compilation, { normalModuleFactory }) => {
				const myLogger = compilation.getLogger ? compilation.getLogger('NodeStuffPlugin') : console;
				const handler = (parser, parserOptions) => {
					...
					const setModuleConstant = (expressionName, fn) => {
						parser.hooks.expression
							.for(expressionName)
							.tap("NodeStuffPlugin", expr => {
								if(!ModuleFilenameHelpers.matchObject(localOptions,parser.state.module.resource))
								{
									myLogger.log('NodeStuffPlugin make no change for ',parser.state.module.resource)
									return true;
								}
								myLogger.info('NodeStuffPlugin make change for ',parser.state.module.resource)
